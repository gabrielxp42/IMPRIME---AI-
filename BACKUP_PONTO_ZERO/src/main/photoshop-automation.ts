...
export class PhotoshopAutomation {
    ...
    constructor() {
        if (app.isPackaged) {
            ...
            const potentialSourcePaths = [
                path.join(process.cwd(), 'src', 'main', 'photoshop_automation.py'),
                path.resolve(__dirname, '..', '..', 'src', 'main', 'photoshop_automation.py'),
                path.join(__dirname, 'photoshop_automation.py')
            ];
            ...
            if (!this.scriptPath) {
                console.error('[Error] Script Python não encontrado em nenhum local de dev!');
            }
        }
    }
    ...
    async openAndResizeImage(filePath: string, widthCm: number, dpi: number = 300): Promise<void> {
        const jsxScript = `
    (function() {
        try {
