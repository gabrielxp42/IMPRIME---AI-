...
def remove_background_advanced(input_path, output_path, remove_internal_blacks=False, black_threshold=30):
    ...
    try:
        ...
        # Usar u2netp (mais rápido) para modo padrão
        model_name = "u2netp"
        try:
        ...
        print(f"PROGRESS:Removendo fundo...", file=sys.stderr, flush=True)
        
        # Remover fundo (alpha matting desativado para velocidade)
        output_image = remove(
            input_image, 
            session=session,
